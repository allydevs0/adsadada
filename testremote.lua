...
if _G.data['SaveDecompileLogs'] then
	...
	print('âœ… - LocalScripts have been dumped! | time taken: ' .. tick() - cur_time .. ' seconds')
end
warn('âš ï¸ - Initializing RemoteHook/main.lua')
...
warn('âœ… - Initialized RemoteHook/main.lua')

if _G.data['SaveScanLogs'] then
	warn('âš ï¸ - Saving scan logs...')
	local cur_time = tick()
	...
	print('âœ… - Scan logs has been saved! | time taken: ' .. tick() - cur_time .. ' seconds')
end
...
Simple.Text = "SimpleSpy lite - (fixed by @allydevs0)"
...
function eventSelect(frame)
    ...
    if selected and selected.Log then
        ...
        -- Mostra o código atualizado com o contador e última vez
...
function newRemote(type, name, args, remote, function_info, blocked, src, returnValue)
    ...
    if groupedRemotes[key] then
        -- Já existe ? atualiza contador e últimos argumentos
        local entry = groupedRemotes[key]
        entry.count = entry.count + 1
        entry.lastFire = now
        entry.args = args          -- mantém os últimos argumentos
        entry.returnValue = returnValue

        -- Atualiza o texto do botão com o novo contador
        ...
        -- Atualiza o código gerado em background (assíncrono)
        schedule(function()
    ...
    -- Define direção (? enviando, ? recebendo). Para simplificar: event/fn = enviando
